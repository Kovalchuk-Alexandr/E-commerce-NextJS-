...
const ProductsPage = ({products}: {products: any[]}) => {
	...
	const getActiveFiltersCount = () => {
		...
		const priceCount = (filter.price[0] !== null || filter.price[1] !== null) ? 1 : 0;
				...
				{getActiveFiltersCount() > 0 && (
					<div className={styles["applied-filters"]}>
						<div className={styles["applied-filters__header"]}>
							<span>Applied Filters: {getActiveFiltersCount()}</span>
							<button
						...
						<div className={styles["applied-filters__tags"]}>
							...
							{/* Цена */}
							{(filter.price[0] !== null || filter.price[1] !== null) && (
								<span className={styles["filter-tag"]}>
									Price: ${filter.price[0] || 0} - ${filter.price[1] || '∞'}
									<button
										onClick={() => setFilter({...filter, price: [null, null]})}
										className={styles["remove-filter-btn"]}
									>
										×
									</button>
